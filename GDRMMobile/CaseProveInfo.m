...
+ (NSString *)generateEventDescForCase:(NSString *)caseID{
    ...
    if (citizenArray.count>0) {
        if (citizenArray.count==1) {
            ...
            caseDescString=[caseDescString stringByAppendingFormat:@"   %@于%@驾驶%@%@行至%@%@%@在公路%@由于%@发生交通事故损坏公路路产，%@，经与当事人现场勘查，",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,case_reason,caseStatusString];
...
+ (NSString *)generateEventDescForInquire:(NSString *)caseID{
    ...
    if (citizenArray.count > 0) {
        ...
        if (citizenArray.count > 1) {
            ...
            caseDescString = [caseDescString stringByAppendingFormat:@"因%@发生碰撞导致损坏公路路产。",caseInfo.case_reason];    
        }
    }
    return caseDescString;
}
