...
- (void) createRecodeByCaseID:(NSString *)caseID{
    ...
    [dateFormatter setDateFormat:DATE_FORMAT_HH_MM_COLON];
    NSString *timeString=[dateFormatter stringFromDate:inspectionRecord.start_time];
    //从案件取时间 格式不符合要求  转一下格式
    NSString *timeStr=[NSString stringWithFormat:@"%@时%@分",[timeString substringToIndex:2] ,  [timeString substringFromIndex:3] ];
   
    
    NSLog(timeStr);
    //改
    NSString *stationstring=[NSString stringWithFormat:@"K%@+%@处",[caseInfo station_start_km], [caseInfo station_start_m]];
