...
//    if ([[[NSString alloc] initWithFormat:@"%02d",self.station_start.integerValue/1000] isEqualToString:@"00"]) {
//        return @"";
//    }
    return [[NSString alloc] initWithFormat:@"%02d",self.station_start.integerValue/1000];
}
- (NSString *)station_start_m{
//    if ([[[NSString alloc] initWithFormat:@"%03d",self.station_start.integerValue/1000] isEqualToString:@"000"]) {
//        return @"";
//    }
...
- (NSString *)roadSegmentAndSide{
    NSString *result=@"";//self.side;
    result = [ [RoadSegment roadNameFromSegment:self.roadsegment_id] stringByAppendingString:self.side];
    return result;
}
...
- (NSString *)full_happen_place{
    ...
    NSString *stationString = [NSString stringWithFormat:@"K%@+%@m",[self station_start_km],[self station_start_m]];
    if ([stationString isEqualToString:@"K00+000m"]) {
        stationString=@"";
    }
    //修改当出现K00+000M时忽略
    // return [NSString stringWithFormat:@"%@%@K%@+%@M%@", roadName, self.side, [self station_start_km], [self station_start_m], self.place];
    return [NSString stringWithFormat:@"%@%@%@%@", roadName, self.side,stationString, self.place];
}
