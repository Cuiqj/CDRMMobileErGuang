...
-(NSString *)getEventDescWithCitizenName:(NSString *)citizenName{
    ...
    if (citizenArray.count>0) {
        if (citizenArray.count==1) {
            ...
            NSString * case_reason = caseInfo.case_reason;
            if([caseInfo.case_reason isEqualToString:@"交通事故"])
            case_reason = @"";
            caseDescString=[caseDescString stringByAppendingFormat:@"于%@驾驶%@%@行至%@%@%@在公路%@由于%@发生交通事故。",happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,case_reason];
            self.caseDescription = [NSString stringWithFormat:@"我于%@驾驶%@%@行至%@%@%@,因%@发生交通事故，导致损坏公路路产。",happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,case_reason];
        }
        if (citizenArray.count>1) {
            for (Citizen *citizen in citizenArray) {
